...
TEST_CASES = [
    ...
    ("TC-E13","パスワード変更 - 確認パスワード不一致エラー","異常系",
     "・パスワード変更画面が表示されている",
     "1. 新しいパスワード欄に「NewPass1!」を入力\n"
    ...
    ("TC-A05","管理者：管理者パスワード変更","正常系",
     "・管理画面が表示されている",
     "1. 管理者パスワード変更欄に「NewAdminPw1!」を入力\n"
     "2. 「変更する」ボタンをクリック\n"
     "3. ログアウトしてログイン画面を開く\n"
     "4. admin / NewAdminPw1! でログインする",
     "1. 管理者パスワード変更成功メッセージが表示される\n"
     "2. 新しいパスワードでログインに成功する\n"
     "3. 管理画面に遷移する",
     "自社ツール（E2E）", "SC-ADM-02", "最高"),

    ("TC-A06","管理者：管理者バッジ表示・操作保護確認","正常系",
     "・管理画面が表示されている",
     "1. ユーザー一覧の admin 行を確認する",
     "1. admin 行に「管理者」バッジが表示される\n"
     "2. admin 行に削除・ロック・無効化ボタンが表示されない",
     "自社ツール（E2E）", "SC-ADM-03", "高"),
    ...
    # ── セッション・セキュリティ ───────────────────────────────────
    ("TC-S01","ログアウト後のセッション無効化確認","セキュリティ",
     "・user001 でログイン済み（ホーム画面表示中）",
     "1. ログアウトボタンをクリックする\n"
     "2. ブラウザの戻るボタンを押してホーム画面への戻りを試みる",
     "1. ログイン画面が表示される\n"
     "2. ホーム画面のコンテンツは表示されない",
     "自社ツール（E2E）", "SC-SEC-01", "高"),

    ("TC-S02","不正なユーザーIDでのログイン試行","セキュリティ",
     "・ログイン画面を開いている",
     "1. ユーザーID欄に「' OR '1'='1」を入力\n"
     "2. パスワード欄に任意の文字列を入力\n"
     "3. ログインボタンをクリック",
     "1. ログインが拒否される（ホーム画面に遷移しない）\n"
     "2. 他のユーザー情報は表示されない",
     "自社ツール（E2E）", "SC-SEC-02", "最高"),

    # ── 境界値（パスワード有効期限・ロック期限） ────────────────────
    ("TC-B01","パスワード変更後89日目のログイン（期限内）","境界値",
     "・e2eexpiry89 が登録されており、パスワード最終変更から89日経過している\n・ログイン画面を開いている",
     "1. e2eexpiry89 / 正しいパスワードでログインする",
     "1. ホーム画面に切り替わる（パスワード変更画面には遷移しない）",
     "自社ツール（E2E）", "SC-BV-PW-89", "中"),

    ("TC-B02","パスワード変更後90日目のログイン（期限切れ）","境界値",
     "・e2eexpiry90 が登録されており、パスワード最終変更から90日経過している\n・ログイン画面を開いている",
     "1. e2eexpiry90 / 正しいパスワードでログインする",
     "1. パスワード変更画面に切り替わる（ホーム画面には遷移しない）",
     "自社ツール（E2E）", "SC-BV-PW-90", "高"),

    ("TC-B03","ロック期限1分以上残っている状態でのログイン拒否","境界値",
     "・e2elock がロック状態で登録されており、ロック解除まで1分以上ある\n・ログイン画面を開いている",
     "1. e2elock / 正しいパスワードでログインを試みる",
     "1. アカウントロック中のメッセージが表示される\n2. ホーム画面へは遷移しない",
     "自社ツール（E2E）", "SC-BV-LOCK-1MIN", "高"),

    ("TC-B04","ロック期限経過後のログイン成功","境界値",
     "・e2elock のロック期限が1分前に経過している\n・ログイン画面を開いている",
     "1. e2elock / 正しいパスワードでログインする",
     "1. ホーム画面に切り替わる（ロックエラーは表示されない）",
     "自社ツール（E2E）", "SC-BV-LOCK-EXPIRED", "高"),
]
...
def generate_excel():
    ...
    tc_hdrs=["テストケースID","テスト名","テスト区分","前提条件（Preconditions）",
             "検証手順（Steps）","期待値（Expected Results）","自動化手段","シナリオID","優先度"]
...
def generate_word():
    ...
    for tc in TEST_CASES:
        ...
        w_table(doc,["属性","内容"],
            [("テスト区分",ttype),("優先度",prio),("自動化手段",tool),("シナリオID",code)],
            [3.5,13.5],hdr_bg="595959")
        doc.add_paragraph()
